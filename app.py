...
def calcular_distancia(lat1, lon1, lat2, lon2):
    try:
        # Garante que as coordenadas são números e não estão vazias
...
try:
    ...
    st.error("Aguardando configuração do arquivo secrets ou requirements.")
...
if menu == "⚙️ Configurar Prova":
    ...
    with col1:
        ...
        c_lat_solta = st.text_input("Latitude Solta (Ex: 39.406522)", key="lat_solta")
        c_lon_solta = st.text_input("Longitude Solta (Ex: -7.432111)", key="lon_solta")
    ...
    if st.button("Gravar Configuração"):
        st.session_state['prova'] = {
            "cidade": cidade, "lat": c_lat_solta, "lon": c_lon_solta,
            "hora": f"{h_s:02d}:{m_s:02d}:{s_s:02d}", "mod": modalidade, "p_ini": p_ini, "dec": dec
        }
        st.success(f"Prova de {cidade} configurada!")

# 2. CADASTRO DE SÓCIOS
elif menu == "👤 Cadastro de Sócios":
    st.header("👤 Cadastro de Pombais")
    with st.form("form_socio"):
        nome_socio = st.text_input("Nome do Sócio / Pombal")
        s_lat = st.text_input("Latitude do Pombal (Ex: 39.123456)")
        s_lon = st.text_input("Longitude do Pombal (Ex: -8.543210)")
        if st.form_submit_button("Salvar Sócio"):
            st.success(f"Sócio {nome_socio} cadastrado!")

# 3. LANÇAR CHEGADAS (3+3)
elif menu == "🚀 Lançar Chegadas (3+3)":
    st.header("🚀 Lançamento de Designados (Milhões)")
    socio_nome = st.text_input("Nome do Sócio")
    ...
    for i in range(1, 7):
        ...
        with c_ani:
            st.text_input("Anilha (Ex: 2004466/26)", placeholder="0000000/26", key=f"ani_{i}")
    ...
    if st.button("Calcular Série"):
        st.success(f"Cálculos para {socio_nome} prontos!")
...
# 4. CLASSIFICAÇÃO
    ...
    if 'prova' in st.session_state:
        ...
        st.write(f"**Coordenadas da Solta:** {p['lat']} / {p['lon']}")
    else:
